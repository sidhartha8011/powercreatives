...
3. Extraction repeatedly forces `private static` helpers to `public` when a caller stays behind (so far: `run_prompt_section`, `nth_link_pos`, `rebuild_heading_html`, `page_state_reply`, `write_page_state`). That is a real, if small, loosening of encapsulation — note it in the class docblock rather than hiding it.
