...
| POST | /seo/sites/{id}/ai/verify | controller.php:~136 | THE LIVE CHECK: fetch the connected site's public robots.txt / llms.txt / llm-info / one page .md as a crawler and explain each answer (card 17) |
...
| POST | /seo/ai-readiness/verify | controller.php:~156 | THE LIVE CHECK for this site (PCM_SEO_AIReadiness::verify_public_files) |
...
| `PCM_Access` | `core/class-pcm-access.php` | Team-access grants via delivery assignments; `scope_clause()` for "owned OR granted" SQL (admins short-circuit to match every row). Also `workspace_api_key()` and `model_scope()` — the two **workspace-resource** levers: keys/models are stored per-owner but a caller who owns none must still see the workspace's (admin → all; owns some → own; else → admin-owned). **Any new model/integration query must use these**: wp-admin resolves the caller to `wp_<ID>` which owns the keys, while the shortcode SPA's gate login is a different platform row owning nothing, so a caller-scoped query silently works in wp-admin and returns zero rows on the public page. Content (projects/brands/assets) deliberately has NO such fallback. **Automation RULES are exempt from the workspace law** (2026-08-12): every user carries an identical seeded set, so a team-wide admin list renders N copies of everything; `list_rules()` shows own rules + foreign admins' CUSTOM rules only (mirrors `run_rules()`'s `__seedKey` gate). |
...
⚠ **`seo/service.php` is down to 1,661 lines / 47 methods** (was 7,135 — **−77%**). What remains is ONE coherent concern — Remote-site SEO (the connector read/write proxy) — plus the deliberate shared seams. A decomposition is IN PROGRESS (started 2026-07-29) following this module's own existing convention — one standalone `PCM_SEO_*` class per concern in its own file, `require_once`'d at the top of `service.php`, called DIRECTLY from `controller.php` (never through a facade). Traits are NOT used anywhere in this plugin — don't introduce them.

**Extracted (9 classes, 112 methods):** `local.php` (`PCM_SEO_Local`, 29 — the hub's OWN posts/pages: cross-plugin meta registry, content rows, link scan/rewrite, headings + content nodes, `run_prompt_section`, `save_cell`), `ai.php` (`PCM_SEO_AI`, 15 — all prompt resolution + field generation; **`language_law()` is the shared language contract — append it to ANY new prompt whose output lands on a page**, or the model answers in the English of its own instructions rather than the page's language. `site.lang` is the hint: for REMOTE sites it comes from the linked brand's scraped `<html lang>`, never `get_locale()`, which describes the hub), `redirects.php` (`PCM_SEO_Redirects`, 7), `business.php` (`PCM_SEO_Business`, 6), `page-state.php` (`PCM_SEO_Page_State`, 6 — page versioning: fingerprint/version per `siteId:postId`), `remote-headings.php` (`PCM_SEO_Remote_Headings`, 6 — H1–H6 on a CONNECTED post: snapshot read, override apply, the three update paths, AI optimize), `page-inventory.php` (`PCM_SEO_Page_Inventory`, 16 — ONE hub-side parse of the connector snapshot: paragraph/heading inventory, rule rows ↔ schema, media push; ZERO promotions needed, only 3 outward edges), `editing.php` (`PCM_SEO_Editing`, 24 — SECTION RULES + THE SAVE TRANSACTION: `save_page_edits`, `save_section_*`, version history, `get_options`; 14 static + **10 INSTANCE** methods, controller holds its own `$this->editing`), `views.php` (`PCM_SEO_Views`, 4).

**Still inside `service.php` (3,761 lines):** Remote-site SEO ~1,557 lines incl. the 738-line `save_page_edits()`, the SAVE TRANSACTION group (12 static methods, ~2,097 lines with SECTION RULES), dynamic rules (1).

⚠ **The two SAVE-TRANSACTION landmines are RESOLVED (2026-07-31 phase 7) — keep them in mind for any future slice:**
1. `private const VERSION_CAP = 20;` was declared INSIDE that block, so it MOVED with it and its refs stayed `self::`. A slicer that blindly qualifies `self::CONST` → `PCM_SEO_Service::CONST` would have pointed at a constant that no longer exists. **Always check whether a constant is declared inside the block you are moving.**
2. The block held **10 INSTANCE methods** (`public function`, reached as `$this->service->x()`), not just statics. They moved as-is because `PCM_SEO_Service` is **stateless** (no properties, no constructor, `$this->` only ever calls siblings) — `controller.php` now declares `private PCM_SEO_Editing $editing;` alongside `$service`. ⚠ The property TYPE HINT must be updated too: leaving `private PCM_SEO_Service $editing;` is a runtime TypeError that `php -l` accepts silently (hit and fixed).
...
**Seams deliberately left on `PCM_SEO_Service`:** `ensure_sites_service()` (`public static`) and the `VALID_TYPES` / `VALID_STATUSES` / `PER_TYPE` constants — `controller.php` already reads the constants from there, so they stay put and `PCM_SEO_Local` references them cross-class. Three former `private static` LOCAL helpers (`run_prompt_section`, `nth_link_pos`, `rebuild_heading_html`) had to become `public` because `service.php` still calls them.

**When adding a new SEO concern, create a new class+file — do not grow `service.php`.**

⚠ **Two gotchas this decomposition hit — check both after any further extraction:**
1. Classes here are **classmap**-autoloaded, so a new file is invisible to PHPUnit until `composer dump-autoload` runs.
2. The test suite does NOT cover most SEO paths, so a moved method left behind by a name-scoped rewrite stays green and fatals only at runtime (this happened once: `$this->build_field_vars()`). Run **`php tests/standalone/seo_decomposition_orphan_check.php`** (exit 1 on failure, CI-usable) — it verifies every moved method is defined on its new class and flags four call shapes still aimed at the old owner: `self::x()`, `$this->x()`, `$var->x()`, `PCM_SEO_Service::x()`, **and string class-names** (`ReflectionMethod('PCM_SEO_Service','x')`, `array('PCM_SEO_Service','x')` callables). Add newly-moved methods to its `$moved` map. Each of those four shapes was added only after it let a real bug through — don't narrow it.
3. ⚠ **VISIBILITY MUST BE RE-DERIVED FROM THE NEW CALL GRAPH AFTER EVERY SLICE — not inherited.** This shipped a production fatal on 2026-07-31: `Call to private method PCM_SEO_Page_Inventory::section_runs() from scope PCM_SEO_Editing`. A moved method keeps its old visibility; that is fine while its callers share the class, but the moment a LATER slice moves those callers to a different class the call becomes cross-class and `private` fatals **at runtime only** — `php -l` accepts it and every test stayed green. Four methods were affected (`section_runs`, `split_unit_sections`, `assemble_content_html`, `rules_to_schema`), and the user hit them one action at a time. Note this is the MIRROR of gotcha 4 below: that one is about stay-behind callees, this one is about MOVED methods whose callers left. `seo_decomposition_orphan_check.php` now has an **"inaccessible cross-class calls"** section that flags any `private`/`protected static` method another seo file calls statically, and exits 1 — verified by re-introducing the bug and watching it name the method.
4. Extraction repeatedly forces `private static` helpers to `public` when a caller stays behind (so far: `run_prompt_section`, `nth_link_pos`, `rebuild_heading_html`, `page_state_reply`, `write_page_state`). That is a real, if small, loosening of encapsulation — note it in the class docblock rather than hiding it.

**AI Readiness UI** (card 17 remake): ONE `app/src/modules/SEO/AiReadinessStudio.tsx` for the local site AND connected sites — three cards (/llm-info/, llms.txt + page .md, robots.txt) each with Generate + Publish, first Generate publishes, "Check live" → `verify_public_files`; prompts are Templates rows (`llm_info_page_generate`, `site_ai_description_generate`, NEW `air_page_summary_generate`) picked per card. The old `AIReadinessPanel` / `RemoteAIReadinessPanel` / `LlmInfoEditor` are gone. **Cross-plugin meta**: `seo_key_map()` / `detect_seo_plugin()` supports Yoast/RankMath/SEOPress + native `pcm_seo_*`. **Schema** (`seo/schema.php`): 6 JSON-LD types (Article, WebPage, BreadcrumbList, FAQPage, HowTo, Product), emitted `wp_head` priority 1. **AI Readiness** (`seo/ai-readiness.php`): virtual `/llms.txt`, `/llms-full.txt`, `/{slug}.md` served via `template_redirect` when published (option `pcm_seo_air_published`); page-builder-aware (Elementor/Divi/Brizy). **GBP** (`seo/gbp.php`): provider behind `PCM_SEO_GBP_Provider` — Google Places (native) or Apify (`compass~crawler-google-places`); stored on the brand's PRIMARY business unit (`brand_business_units`), fetched layer + manual overrides.
...
- **`cn()` runs tailwind-merge, so a CALL-SITE class BEATS the shared component's base.** A shared component therefore has no authority over any token a call site also sets — this is how 129 `<SelectTrigger>` sites drifted to 4 heights/2 fonts/5 backgrounds despite a perfectly good `variant` API. **Dropdown law**: height/font/background/border/shadow/focus-ring belong to `selectTriggerVariants` (`components/ui/select.tsx`, canon `h-8 / text-xs / bg-card`; `variant="ghost"` for chromeless, `size="auto"` to opt out of the height); call sites pass **width and layout only**. Guarded by `tests/standalone/select_style_consistency_test.mjs`. Note the canonical heights are attribute-prefixed (`data-[size=…]:h-8`) and so outrank a plain `h-full` on specificity — hence `size="auto"` rather than a class the call site could never win with.
