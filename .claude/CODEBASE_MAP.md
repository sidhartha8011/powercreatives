...
- **Frontend** `app/src/modules/Keywords/CreateStrategyDialog.tsx` (opened from the Keywords
  ...
  Writer template, **AI Model** (2026-07-08 — a text-model dropdown from
  `trpc.models.getForGeneration({type:'text'})`, empty = default; sends `model`+`provider`),
  `structure`, content hierarchy (+ parent URL/keyword), auto-interlink (+ max links),
  publishing mode, approval mode, schedule (frequency/start date). **All fields now persist**
  into the strategy `config`; the interlink/schedule/approval/hierarchy ones are stored-but-not-
  yet-executed (see *What's still missing*).
- **Frontend** `app/src/modules/Strategies/index.tsx` (flat file): collapsible strategy cards
  ...
  `generate` with `{itemId}`). The per-item "View" button is still `onClick`-less (dead). No
  calendar/kanban view. `docs/modules/strategy.md` + a roadmap doc still describe a larger
  planned system (Workflows tie-in, approval gate, WP-Cron scheduling, interlink injection,
  `review`/`scheduled`/`published` item states) that remains unbuilt.
- **What's still missing** (deferred as feature-sized follow-ups needing their own cron/publish/
  ...
  auto-publish to a connected site, approval-gate wiring to the Approvals module, interlink
  injection between generated articles, hierarchy-aware (pillar/cluster) generation, and the
  `structure='consolidated'` mode. **Keyword research + the core strategy→item→LLM→article loop
