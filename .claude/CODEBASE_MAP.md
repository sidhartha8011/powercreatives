...
| DB version (`PCM_DB_VERSION`) | **1.36.0** (separate from plugin version. 1.28.0 `deliveries.seoSiteId`; **1.29.0** `projects.deliveryId` (Brand→Delivery→Project, additive + backfill `class-pcm-schema.php:1033`); **1.30.0** seeded a "email client review link on share" automation; **1.31.0** removes that rule — it double-sent vs the built-in dispatch path — idempotent, `class-pcm-activator.php:282`; **1.32.0** adds `externalId varchar(191)` to brands/deliveries/projects — backs the webhook `brandExtID`/`deliveryExtID`/`projectExtID` tokens; **1.33.0** further additive dbDelta; **1.34.0** `siteId` FK on `projects` + `articles` (Site↔Project connection + the strategy auto-publish target); **1.35.0** DEPRECATES the write-only `deliveries.seoSiteId` (site now resolves live via `PCM_Hierarchy`; do not rewire) + adds `wp_pcm_delivery_logs` (free-text work log per delivery); **1.36.0** `delivery_assignments.role` (`'lead'|'member'` — a lead is cosmetic; access/grants ignore role). 1.34–1.36 all additive, landed via the 2026-07-07 `origin/feat/seo-suite-port` merge.) |
...
| strategy | pcm/v1/strategies | Content-strategy pipeline: keyword list → per-item AI article generation → `pcm_articles`. See *Content Strategies pipeline*. |
...
## Content Strategies pipeline (`strategy` module, re-verified 2026-07-08)
- **What it is:** create a "strategy" from a list of keywords (picked in the **Keywords**
  module) + a Writer template + optional brand → one `strategy_items` row per keyword →
  a manually-triggered, **one-item-per-call** LLM generation into `pcm_articles`. It's a
  real working pipeline, not a stub — but materially narrower than its own creation UI implies.
- **Backend** (`includes/modules/strategy/{controller,service}.php`, `pcm/v1/strategies`,
  `edit_posts` + delivery grant key `strategies`): `POST /strategies` (`create_from_keywords`)
  reads `name, templateId, brandId, keywords[], hierarchyMode, publishingMode` **plus** builds
  a `config` JSON from `model, provider, structure, approvalMode, parentTargetUrl,
  parentKeyword, interlinksConfig, scheduleConfig` (2026-07-08 — these were previously dropped)
  → inserts the `strategies` row + one `strategy_items` row per keyword (`status='pending'`).
  `POST /strategies/{id}/generate` (`generate_next_item($strategy,$uid,$item_id=null)`) picks
  the next `status='pending'` item by `position` — **or a specific item when the body carries
  `{itemId}`** (retry/regenerate; any status allowed, but the item must belong to the strategy).
  Loads the template's prompt + brand context, calls `PCM_LLM::invoke_json` with the
  **user-selected `model`/`provider` from the strategy's config** (data-driven; falls back to
  `gemini-2.5-flash` with no explicit provider for strategies created before model selection —
  see `resolve_model()`), inserts a `pcm_articles` row, marks the item `completed`/`error`, and
  **recomputes `completedItems`/`failedItems`/`status` from the actual item states**
  (`recompute_counters()` — retry-safe: a re-generated error→completed item isn't double-counted).
  Still **no server-side batch/queue runner or cron** — each call advances one item.
- **DB** (`class-pcm-schema.php:363-406`): `strategies` (name/status/hierarchyMode/
  publishingMode/config/totalItems/completedItems/failedItems) + `strategy_items`
  (keyword/title/slug/status/articleId/position/errorMessage). `strategies.config` now holds
  the generation config (model/provider are READ at generate time; the interlink/schedule/
  approval/hierarchy fields are stored but not yet acted on). `strategy_items.config` is still
  never written (`create_strategy_items` inserts only strategyId/userId/keyword/position/status).
  **`config.siteId`** (2026-07-08, int, `absint`'d) is the site auto-publish targets — see below.
- **Auto-publish** (2026-07-08): `generate_next_item()` calls a new `maybe_auto_publish()` after
  article creation + counter recompute — when `strategy->publishingMode === 'publish'` (a top-level
  column, not in `config`) and `config.siteId` resolves via ownership-scoped `PCM_DB::get_site()`,
  it reuses `PCM_Sites_Service::publish_to_site()` verbatim (the same call the Sites module's own
  `/publish` route makes — no new WP REST logic). A publish failure is isolated from generation
  state: it never touches `item.status`/`errorMessage`/the completed-failed counters (those mean
  "generation outcome," not "publish outcome") — instead it surfaces as a `publish:{success,message}`
  key in the generate response, which the frontend toasts. `require_once` for the cross-module
  service.php is guarded with `class_exists('PCM_Sites_Service')` (harmless, standard idiom;
  behavior-neutral in production) so a unit harness can substitute a fake — the real
  `publish_to_site()` needs `wp_salt()` + genuinely-encrypted app-password data, too heavy to stub
  faithfully just to verify the new orchestration wrapper.
- **Frontend** `app/src/modules/Keywords/CreateStrategyDialog.tsx` (opened from the Keywords
  module's bulk-action bar — Strategies' own `index.tsx` has no "create" button) collects: name,
  **Target Site** (2026-07-08 — `trpc.sites.list`, a first-class ALWAYS-visible selector at the top,
  defaults to the first connected site, mirrors AutoPress where a strategy is always bound to a site;
  sends `siteId` on every strategy regardless of publishing mode — REQUIRED (Create disabled + inline
  hint) only when publishingMode==='publish' so auto-publish can't silently no-op), Writer template,
  **AI Model** (2026-07-08 — a text-model dropdown from `trpc.models.getForGeneration({type:'text'})`,
  empty = default; sends `model`+`provider`), `structure`, content hierarchy (+ parent URL/keyword),
  auto-interlink (+ max links), publishing mode, approval mode, schedule (frequency/start date).
  **All fields persist** into the strategy `config`; the interlink/schedule/approval/hierarchy ones
  are still stored-but-not-yet-executed (see *What's still missing*). `siteId` is fully wired end to
  end (stored on every strategy; `maybe_auto_publish` still only publishes in publish mode, so a draft
  carrying a siteId stays a draft — verified). NB earlier same-day the site picker was publish-gated
  ("Site to Publish To", only shown when publishingMode==='publish") — that regressed AutoPress's
  always-select-a-site model and was corrected to the always-visible Target Site above.
- **Frontend** `app/src/modules/Strategies/index.tsx` (flat file): collapsible strategy cards
  (name, status badge, `{completed}/{total}` + failed count, progress bar) with **"Generate All"**
  (2026-07-08 — client-side loop over the per-item endpoint until `complete`, single summary
  toast, `totalItems+1` runaway guard), single **"Generate"** (next pending), and delete.
  Expanded items list each row with status; **errored items get a "Retry" button** (2026-07-08 —
  `generate` with `{itemId}`); a completed item's **"View" button now opens the article in Writer**
  (2026-07-08 — was `onClick`-less/dead; see *Cross-module: open an article in Writer* below). A
  publish failure (from auto-publish) toasts even during a bulk "Generate All" run, since it isn't
  otherwise counted in that run's summary. No calendar/kanban view. `docs/modules/strategy.md` + a
  roadmap doc still describe a larger planned system (Workflows tie-in, approval gate, WP-Cron
  scheduling, interlink injection, `review`/`scheduled`/`published` item states) that remains unbuilt.
- **Cross-module: open an article in Writer** (2026-07-08) — `AppContext.tsx` gained a 4th instance
  of the established one-shot nav pattern (mirrors `navigateToProjectTab`/`navigateToApprovalsWithSet`
  exactly): `pendingWriterArticleId` + `navigateToWriterArticle(articleId)` +
  `consumePendingWriterArticleId()`. `Writer/index.tsx` consumes it in a `useEffect` keyed on
  `writerDocs` (NOT on the persistence hook's `isLoading` flag — that flips false one render BEFORE
  the loaded articles actually land in `writerDocumentsAtom`, which would otherwise consume-and-lose
  the one-shot id before the match could be found; only acts at the exact moment the target doc is
  confirmed present in `writerDocs`, so a genuinely-missing article just leaves the pending id inert
  rather than mis-selecting anything).
- **What's still missing** (deferred as feature-sized follow-ups needing their own cron/
  Approvals architecture): AI-suggested topics (items are still 1:1 with pre-picked keywords),
  real scheduling EXECUTION (schedule config is stored, not run — no cron/due-date processor),
  approval-gate wiring to the Approvals module, interlink injection between generated articles,
  hierarchy-aware (pillar/cluster) generation, and the `structure='consolidated'` mode.
  Auto-publish (above) and the Writer "View" link are DONE, no longer on this list.
  **Keyword research + the core strategy→item→LLM→article loop
  already work — don't rebuild them** (keyword research: Google Suggest + Ahrefs volume/KD/SERP-DR
  via the Ahrefs MCP endpoint, see `keywords` module above; no DataForSEO integration exists).
