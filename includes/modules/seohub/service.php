...
add_action('template_redirect', function () {
    ...
    $slug  = urldecode(substr($path, 0, -3));
    $types = array_values(get_post_types(array('public' => true)));
    // Hierarchical path first (pages, nested pages); then the LAST segment by name for
    // posts behind dated / category permalinks (/2024/05/my-post.md) — the hub's own
    // serve does the same. Before this, every non-top-level URL was a 404.
    $page = get_page_by_path($slug, OBJECT, $types);
    if (!$page) {
        $found = get_posts(array('name' => basename($slug), 'post_type' => $types, 'post_status' => 'publish', 'numberposts' => 1));
        $page  = !empty($found) ? $found[0] : null;
    }
    if (!$page || $page->post_status !== 'publish') { status_header(404); nocache_headers(); echo '# Page not found'; exit; }
    status_header(200);
    header('Content-Type: text/markdown; charset=utf-8');
    header('X-Robots-Tag: index, follow');
    echo '# ' . html_entity_decode(get_the_title($page), ENT_QUOTES, 'UTF-8') . "\n\n" . pcm_conn_html_to_md(strip_shortcodes($page->post_content));
    exit;
});
// .md / llms.txt are virtual files: never let canonical redirection turn them into /slug/ or strip them.
add_filter('redirect_canonical', function ($redirect_url, $requested_url) {
    $p = (string) parse_url((string) $requested_url, PHP_URL_PATH);
    return preg_match('#\.(md|txt)$#i', $p) ? false : $redirect_url;
}, 10, 2);
