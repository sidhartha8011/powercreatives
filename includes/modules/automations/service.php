...
class PCM_Automation_Engine
{
     ...
     * @return bool
     */
    private static function already_sent(string $dedupe_key, int $user_id): bool
    {
        global $wpdb;
        ...
        $count = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$table}
             WHERE userId = %d AND dedupeKey = %s AND status = %s",
            $user_id,
            $dedupe_key,
            'sent'
        ));
     ...
     * @param array  $context    Trigger context (provides condition + payload values).
     * @param int    $user_id    Owning PCM user id.
     * @return array<int, array> Per-rule action results (for tests/introspection).
     */
    public static function fire_trigger(string $trigger_id, array $context, int $user_id): array
    {
        self::boot();
        ...
        $rows = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, conditions, actionId, config, inputMapping, brandId FROM {$table}
                 WHERE userId = %d AND triggerId = %s AND isActive = 1",
                $user_id,
                $trigger_id
            )
        ) ?: array();
        ...
        foreach ($rows as $row) {
            ...
            $results[] = self::run_action((string) $row->actionId, $config, $mapping, $trigger_id, $context, $user_id, (int) $row->id);
        }
     ...
     */
    private static function run_action(string $action_id, array $config, array $mapping, string $trigger_id, array $context, int $user_id, int $rule_id): array
    {
        ...
        if (!$handler) {
            ...
            self::log_action($user_id, $rule_id, $trigger_id, $action_id, $result, $context);
        ...
        if ($handler->mode() === 'async' && function_exists('wp_schedule_single_event')) {
            ...
            self::log_action($user_id, $rule_id, $trigger_id, $action_id, $result, $inputs ?: $ctx);
     ...
     * @return void
     */
    private static function log_action(int $user_id, int $rule_id, string $trigger_id, string $action_id, array $result, array $payload): void
    {
        global $wpdb;
        $table = PCM_Schema::table('automation_logs');
    ...
    public static function update_rule(int $id, int $user_id, array $data): bool
    {
        ...
        if (array_key_exists('config', $data)) {
            $update['config'] = wp_json_encode($data['config'] ?? array());
        }
