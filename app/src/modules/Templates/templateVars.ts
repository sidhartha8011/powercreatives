...
const WRITER_VARS: Record<string, string> = {
  ...
  '{{ supporting_keyword }}': 'The other keywords of a consolidated (one-article) strategy, comma-separated. Empty for a single-keyword item.',
  '{{ post_type }}': 'What the article publishes as — always “post” for strategy articles (SEO templates’ name for it).',
  '{{ page.type }}': 'SEO’s page-intent name for this content — “blog” for a strategy article.',
  '{{ name }}': 'The business name, as SEO templates call it (same value as {{business.name}}).',
  '{{ facts }}': 'The business facts block SEO templates get (name, website, category, address, phone, hours, rating, about) — only the details that are set. Empty when nothing is known.',
  // FULL PARITY with SEO (owner: "all the variables including meta and everything").
  // Every SEO token is accepted here so an SEO prompt pastes into a Writer template
  // unchanged. Values a new article cannot have yet resolve to nothing — said so.
  '{{ topic }}': 'The subject to write about — the item’s keyword (SEO’s name for the section topic).',
  '{{ keywords_bullet }}': 'Ready-made “weave in the target keywords” bullet, set whenever the item has a keyword.',
  '{{ meta_title }}': 'Empty for a new article — the meta title is written by the generation itself. Accepted so SEO prompts paste unchanged.',
  '{{ meta_description }}': 'Empty for a new article — the meta description is written by the generation itself.',
  '{{ meta_keywords }}': 'Empty for a new article — no meta keywords exist before it is written.',
  '{{ current_value }}': 'Empty here — SEO’s “existing value being rewritten”; a new article has nothing to optimise yet.',
  '{{ why }}': 'Empty here — SEO’s one-sentence change explanation only exists when rewriting an existing field.',
  '{{ key_pages }}': 'Empty here — the SEO llm-info page’s list of key pages; not part of a strategy item.',
  '{{ corpus }}': 'Empty here — the SEO llm-info page’s site-content corpus; not part of a strategy item.',
  '{{ corpus_note }}': 'Empty here — the ready-made “base it on the site content” note that goes with {{ corpus }}.',
  '{{ strengths_bullet }}': 'Empty here — comes from the SEO Business form’s strengths field.',
  '{{ years_bullet }}': 'Empty here — comes from the SEO Business form’s years-in-business field.',
  '{{ area_bullet }}': 'Empty here — comes from the SEO Business form’s service-area field.',
  '{{ area_section }}': 'Empty here — the extra “areas we serve” section, from the SEO Business form’s service area.',
  '{{ area_serves_clause }}': 'Empty here — the “serves …” clause, from the SEO Business form’s service area.',
