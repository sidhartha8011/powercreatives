...
export function PreviewDialog({ set, url, onClose }: PreviewDialogProps) {
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent
        ref={setContentEl}
        /* The Radix content is a transparent viewport host. The child below is
           still the exact 95vw × 90vh popup; separating host geometry from
           visual geometry lets a nested document use a real viewport overlay
           without turning the preview itself into a full-screen surface. */
        unstyled
      ...
      >
        ...
        {/* "Open in new tab" — positioned to the left of the auto-rendered
          ...
          <DialogClose
            className="ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute top-4 right-4 z-50 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden"
