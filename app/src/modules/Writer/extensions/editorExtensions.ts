/**
 * WRITER MODULE — Tiptap Editor Extensions Registry
 *
 * Single source of truth for all Tiptap extensions used in the Writer editor.
 * Centralizes configuration so ReviewEditorCanvas.tsx stays lean and
 * new extensions can be added/configured in one place.
 *
 * Architecture:
...
/**
 * Returns the full, configured Tiptap extension array for the Writer editor.
 *
    ...
    FileHandler.configure({
      ...
      onPaste: (_editor, files) => {
        if (options?.onImageFiles) {
          ...
          console.log('[Writer] Image pasted — upload handler not configured yet', files);
        }
      },
      onDrop: (_editor, files) => {
        if (options?.onImageFiles) {
          ...
          // eslint-disable-next-line no-console
          console.log('[Writer] Image dropped — upload handler not configured yet', files);
        }
      },
    }),
  ];
