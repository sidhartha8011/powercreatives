/**
 * REFERENCE ADS SECTION
 *
 * Renders a dynamic list of reference ad textareas with add/remove controls.
 * Replaces the old static competitor_ad_1/competitor_ad_2 fields.
 *
 * Form values are stored as indexed keys: reference_ad_0, reference_ad_1, …
 * This keeps the flat Record<string, string> structure compatible with the
 * existing config-driven architecture.
 *
 * Features:
 *   - Dynamic add/remove with +/- buttons
 *   - Auto-populated from template entries (via parent)
 *   - Collapsible section header (matches DynamicSection style)
 ...
 */
export function countFilledReferenceAds(values: CopyFormValues): number {
...
export function ReferenceAdsSection({
  ...
  return (
    <div className="rounded-lg border" style={{ borderColor: "#e5e7eb" }}>
      {/* Section Header — matches DynamicSection style */}
      <button
        type="button"
      ...
      {!isCollapsed && (
        <div className="p-3 pt-2 space-y-2">
          ...
          <p className="text-[10px]" style={{ color: "#999" }}>
            ...
            Auto-populated when a template is selected.
          </p>
          ...
          {entries.length === 0 && (
            <p
              ...
              style={{ color: "#999" }}
            >
              No reference ads yet. Click + to add one, or select a template to
              auto-populate.
            </p>
          )}
