                ...
                {runDirectives ? (
                  <div title={s.genModel ? `Generated by ${s.genModel}` : undefined}>
                    <div className="mt-0.5 flex flex-wrap gap-1">
                      {Array.from(new Set(runDirectives.flatMap((d) => d.purposes))).map((p) => (
                        <span key={p} className="rounded-full bg-[#e7f5ff] px-1.5 py-px text-[8px] font-semibold text-primary">
                          {TEACHER_PILLS[p] ?? p.toUpperCase()}
                        </span>
                      ))}
                    </div>
