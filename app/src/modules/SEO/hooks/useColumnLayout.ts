/**
 * useColumnLayout — spreadsheet-style column order + widths for the SEO content
 * table, persisted to localStorage (per browser; user picked this over a
 * server-side store). Widths are px; order is a list of column keys.
 *
...
const STORAGE_KEY = 'pcm:seo:col-layout:v1';
...
function loadLayout(keys: string[], defaults: Record<string, number>): ColumnLayout {
...
export function useColumnLayout(
  ...
  const [layout, setLayout] = useState<ColumnLayout>(() => loadLayout(columnKeys, defaultWidths));
